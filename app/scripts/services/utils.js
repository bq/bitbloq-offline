...
angular.module('bitbloqOffline')
    .service('utils', function(_, $q, $timeout) {
        ...
        exports.removeDiacritics = function(str, config) {
        ...
        exports.isNewVisit = function(object, userId) {
            var visit = false;
            if (userId) {
                if (object.creatorId && object.creatorId !== userId) {
                    visit = true;
                    var shareArray = _.keys(object._acl);
                    shareArray.forEach(function(acl) {
                        if (acl === 'user:' + userId) {
                            visit = false;
                        }
                    });
                } else if (object.creator && object.creator.id !== userId) {
                    visit = true;
                }
            }
            return visit;

...
angular.module('bitbloqOffline')
    .controller('PythonSoftwareTabCtrl', function($scope, common, bloqs, $translate, $rootScope, $document, $log, $window, web2board) {
        $log.debug('PythonSoftwareTabCtrl', $scope.$parent.$id);

        $scope.init = function() {
            if ($scope.pythonBloqs.pythonMainBloq) {
                bloqs.removeBloq($scope.pythonBloqs.pythonMainBloq.uuid, true);
                $scope.pythonBloqs.pythonMainBloq = null;
            }

            $scope.pythonBloqs.pythonMainBloq = bloqs.buildBloqWithContent($scope.project.software, $scope.componentsArray, bloqsSchemas, $scope.$field);

            $scope.$field.append($scope.pythonBloqs.pythonMainBloq.$bloq);
            $scope.pythonBloqs.pythonMainBloq.enable(true);
            $scope.pythonBloqs.pythonMainBloq.doConnectable();
